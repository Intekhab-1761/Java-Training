...
public class ForEachApp {
	...
	public static void main(String[] args) {
		...
		List<Product> cereals = List.of(
				...
				new Product("Fruity Oatmeal", "Old Oat", 6.99, 4.3),
		...
		smartWatches.put(2, new Product("Berry watch", "HVR", 159.99, 4.7));
		smartWatches.put(3, new Product("A1 Model", "MorePro", 139.99, 4.3));
		smartWatches.put(4, new Product("A2 Model", "More Pro", 149.99, 4.4));
	
		smartWatches.entrySet().stream()
		.filter(product -> product.getValue() != null &&
				product.getValue().getBrand().equals("MorePro"))
		.forEach(productEntry -> {
			Product productValue = productEntry.getValue();
			System.out.println(productValue.getName() + "-" + productValue.getBrand() +"- $" + productValue.getPrice());
		});
	}
